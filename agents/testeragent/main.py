...
class TesteragentAgent(AgentBase):
    """
    ...
    async def initialize(self):
        ...
        self.logger.info("🔧 Initializing Testeragent Agent...")
        ...
        if self.shared_context:
            await self.shared_context.set("testeragent_status", "initialized")
        
        self.logger.info("✅ Testeragent Agent initialized")
    
    async def execute(self):
        """Main agent execution logic"""
        self.counter += 1
        self.logger.info(f"🔄 Testeragent Agent execution #{self.counter}")
        ...
        if self.counter % 5 == 0:
            self.broadcast_message({
                ...
                "message": f"Testeragent Agent has executed {self.counter} times"
            })
    ...
    async def cleanup(self):
        ...
        self.logger.info("🧹 Cleaning up Testeragent Agent...")
        
        # Example: Update shared context
        if self.shared_context:
            await self.shared_context.set("testeragent_status", "stopped")
        
        self.logger.info("✅ Testeragent Agent cleaned up")
